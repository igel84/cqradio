		...
		return this.each(function(i){
			...
			.each(function(j){
				...
				jqimg.click(function(){
					//открытие в новой вкладки полной картинки
					//window.open(jqchild.find('span').text());
					//var next = n.index + 1;
				...
				image.onload = function(){
					image.onload = null;
					$loader.fadeOut();
					//отступы для внутренней картинки
					jqimg.css({marginLeft:-image.width*.55,marginTop:-image.height*.25,position:'absolute',left:'50%',top:'30%'}).fadeIn();
				...
				$(jqimg).wrap('<a href="'+jqchild.find('span').text()+'" class="fancyBox">');
